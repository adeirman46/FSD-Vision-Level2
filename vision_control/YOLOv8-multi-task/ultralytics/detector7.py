...
while True:
    ...
    if zed.grab() == sl.ERROR_CODE.SUCCESS:
        ...
        # Run YOLOv8 inference ---- bitwise_frame -> frame_rgb
        ...
        # Process and display masks
        if len(plotted_img) > 1:
            im0 = plotted_img[0].copy()
            ...
            for i in range(1, len(plotted_img)):
                ...
                alpha = 0.5
                im0[np.any(color_mask != 0, axis=-1)] = (1 - alpha) * im0[np.any(color_mask != 0, axis=-1)] + alpha * color_mask[np.any(color_mask != 0, axis=-1)]

            # Display the image with masks
            cv2.imshow("ZED + YOLOv8 - Segmentation", cv2.cvtColor(im0, cv2.COLOR_RGB2BGR))
        ...
        # Display the image
        cv2.imshow("ZED + YOLOv8 - Object Detection", cv2.cvtColor(frame_rgb, cv2.COLOR_BGR2RGB))
