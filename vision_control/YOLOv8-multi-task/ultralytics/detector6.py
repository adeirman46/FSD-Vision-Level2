...
while True:
    ...
    if zed.grab() == sl.ERROR_CODE.SUCCESS:
        ...
        for result in results:
            ...
            # im0[np.any(color_mask1 != [0, 0, 0], axis=-1)] = (1 - alpha) * im0[np.any(color_mask1 != [0, 0, 0], axis=-1)] + alpha * color_mask1[np.any(color_mask1 != [0, 0, 0], axis=-1)]
            # im0[np.any(color_mask2 != [0, 0, 0], axis=-1)] = (1 - alpha) * im0[np.any(color_mask2 != [0, 0, 0], axis=-1)] + alpha * color_mask2[np.any(color_mask2 != [0, 0, 0], axis=-1)]
                
            # cv2.imshow("Segmentation: ", im0)  
