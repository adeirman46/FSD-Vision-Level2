...
kalman_filters = {}
lane_kalman_filter = KalmanFilter(process_variance=0.01, measurement_variance=0.1, initial_value=0)
...
while running:
    ...
    if zed.grab() == sl.ERROR_CODE.SUCCESS:
        ...
        plotted_img = []
        lane_distances = []
        speed = 0  # Initialize speed to avoid undefined variable error
        for result in results:
            if isinstance(result, list):
                ...
                # Identify lane segmentation masks
                if masks is not None:
                    for mask in masks:
                        # Find the farthest point in the lane segmentation
                        coords = np.column_stack(np.where(mask))
                        if len(coords) > 0:
                            for coord in coords:
                                depth_value = depth.get_value(coord[1], coord[0])[1]
                                if np.isfinite(depth_value):
                                    lane_distances.append(depth_value)
            else:
                plotted_img.append(result)

        if lane_distances:
            farthest_distance = max(lane_distances)
            filtered_lane_distance = lane_kalman_filter.update(farthest_distance)

            # Update brake and velocity control based on the farthest point in lane segmentation
            brake_value = brake_control(filtered_lane_distance)
            velocity_value = velocity_control(filtered_lane_distance)

            # Display lane distance information
            lane_distance_label = f'Farthest lane distance: {filtered_lane_distance:.2f}m'
            cv2.putText(frame_rgb, lane_distance_label, (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
